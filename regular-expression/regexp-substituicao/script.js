...
'PHP e Java são linguagens diferentes'.replace(regexp, '--$&Script'); // ao utilizar $ é uma referencia a palavra utilizada no regexp
...
const regexp4 = /(ta)+/gi; // seleciona o ta como uma unica palavra
...
const regexp6 = /\d(?=px)/g; // o (?=px) seleciona os valores que estão antes do px, o interresante é que ele não seleciona o px
...
const regexp7 = /\d(?!px)/g; // o (?!px) seleciona todos os digitos que não tiverem px na frente
...
const regexp8 = /(?<=R\$)[\d]+/g; // seleciona os valores que estiverem após o R$
