...
// o json é independente da linguagem que ira interpretar ele
 const exJson = {
  "id": 1,
  "nome": "Andre",
...
const valoresJSON = {
  ...
  "pertences": [
    "lapis",
    "caneta",
    "caderno" // IMPORTANTE, jamais colocar virgula no ultimo item tanto da array ou tanto do objeto
  ],
  "endereco": {
...
const Json = [ // varias array´s com o valor de cada uma sendo um objeto
  {
    id: 1,
    aula: 'JavaScript',
    tempo: '25min',
...
const textoOBJ = JSON.parse(textoJSON); // transformando um texto JSON em um objeto JS
...
const enderecoJSON = JSON.stringify(enderecoOBJ); // transformando um objeto em um JSON
...
localStorage.configuracoes = JSON.stringify(configuracoes); // passo apenas uma chave com os valores de configuração
const pegarConfiguracoes = JSON.parse(localStorage.configuracoes); // pegando as configurações
